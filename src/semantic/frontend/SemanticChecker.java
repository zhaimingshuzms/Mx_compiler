...
public class SemanticChecker implements ASTVisitor{
    ...
    public void visit(RootNode node) {
        ...
        for (var i : node.strDefs)
            ...
            else throw new semanticError("Rootnode semantic.util.error", node.pos);
        if (!checkMain) throw new semanticError("no main semantic.util.error", node.pos);
    ...
    public void visit(memberExprNode node){
        ...
        else throw new semanticError("memberExprNode semantic.util.error",node.pos);
    ...
    public void visit(funcExprNode node){
        ...
        if (node.expression.exprType!=ExprNode.ExprType.FUNCTION){
            ...
            else throw new semanticError("func expression instance semantic.util.error",node.pos);
    ...
    public void visit(binaryExprNode node){
        ...
        if (op== binaryExprNode.binaryOpType.sub||
            ...
            else throw new semanticError("BinaryExprNode semantic.util.error 1",node.pos);
        }
            ...
            else throw new semanticError("BinaryExprNode semantic.util.error 2",node.pos);
        }
        ...
        else throw new semanticError("BinaryExprNode semantic.util.error 3",node.pos);
    }
    ...
    public void visit(prefixExprNode node){
        ...
        if (op==prefixExprNode.prefixOpType.Plus||op== prefixExprNode.prefixOpType.Minus||op== prefixExprNode.prefixOpType.Tilde){
            ...
            if (node.expr.exprType!= ExprNode.ExprType.LVALUE){
                throw new semanticError("PrefExprNode semantic.util.error",node.expr.pos);
            }
            IntType intType=new IntType();
